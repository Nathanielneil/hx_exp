...
// 创建ROSbag对象
rosbag::Bag bag_ADRC;
// bool bag_open_flag = false;
// bool bag_close_flag = false;
// rosbag::Bag bag_UAVpos;
// rosbag::Bag bag_payloadpos;
// string file_path = "/bagfiles/UAVstate.bag";
// string path = (getenv("HOME")+file_path).c_str();
// bag_UAVpos.open(path, rosbag::bagmode::Write);
bool bag_write_flag = false;
...
UAV_controller::UAV_controller(ros::NodeHandle &nh)
{
    ...
    else if (pos_controller == POS_CONTOLLER::ADRC)
    {
        // 【控制器】ADRC控制器初始化
        pos_controller_adrc.init(nh);
        cout << YELLOW << node_name << " Using the ADRC controller... " << TAIL << endl;
    }
    else
    {
    ...
    //【订阅】负载位置信息
    /*uav_payload_sub = 
        nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/uav1_load/pose",
                                                 1,
                                                 &UAV_controller::uav_payload_cb, this);*/
...
void UAV_controller::mainloop()
{
    if (control_state == CONTROL_STATE::RC_POS_CONTROL || control_state == CONTROL_STATE::COMMAND_CONTROL)
    {
        ...
        else if (safety_flag == 1)
        {
            ...
            // 在COMMAND_CONTROL模式下，变更Hover_position与Hover_yaw的值，转到RC_POS_CONTROL模式
            if (control_state == CONTROL_STATE::COMMAND_CONTROL)
            {
                Hover_position = uav_pos;
                Hover_yaw = uav_yaw;
                control_state = CONTROL_STATE::RC_POS_CONTROL;
                cout << RED << uav_name << "Switch to manual control mode!" << TAIL << endl;
            }
            // 在RC_POS_CONTROL模式下，由于已经在set_hover_pose_with_rc函数中约束了Hover_position的范围，因此不做处理
        }
        else if (safety_flag == 2)
        {
            ...
            uav_control_state.failsafe = true;
            control_state = CONTROL_STATE::LAND_CONTROL;
            cout << RED << uav_name << "Switch to land control mode!" << TAIL << endl;
        }
    ...
    case CONTROL_STATE::COMMAND_CONTROL:
        ...
        {
            // 注意：目前内置的位置环控制器,如PID,UDE,NE,ADRC仅支持位置控制(XYZ_POS)及轨迹控制(TRAJECTORY)
...
Eigen::Vector4d UAV_controller::get_cmd_from_controller()
{
    ...
    if (pos_controller == POS_CONTOLLER::PID)
    {
        ...
        // 控制器更新
        Eigen::Vector4d u_att_from_pos_controller = pos_controller_pid.update(100.0);
    ...
    else if (pos_controller == POS_CONTOLLER::UDE)
    {
        ...
        // 控制器更新
        Eigen::Vector4d u_att_from_pos_controller = pos_controller_ude.update(100.0);
        return u_att_from_pos_controller;
    }
   /* else if (pos_controller == POS_CONTOLLER::NE)
    {
        ...
        // 传入控制命令模式
        pos_controller_ne.set_control_command(uav_command);
        // 传入遥控器控制模式
        pos_controller_ne.set_control_state(uav_control_state);
        // 控制器更新
        Eigen::Vector4d u_att_from_pos_controller = pos_controller_ne.update(100.0);
        // // 遥控器拨杆从中间往下拨时，打开bag文件；反之则关闭bag文件
        // if (last_control_state == CONTROL_STATE::RC_POS_CONTROL && control_state == CONTROL_STATE::COMMAND_CONTROL)// && bag_open_flag == false)
        // {
        //     string file_path = "/bagfiles/ADRC_State.bag";
        //     string path = (getenv("HOME")+file_path).c_str();
        //     bag_ADRC.open(path, rosbag::bagmode::Write);
        //     // bag_open_flag == true;
        // }
        // if (last_control_state == CONTROL_STATE::COMMAND_CONTROL && control_state == CONTROL_STATE::RC_POS_CONTROL)
        //     bag_ADRC.close();
        // // 在Move模式下，写入ESO变量追踪的话题至bag文件
        // if (uav_command.Agent_CMD == prometheus_msgs::UAVCommand::Move)
        // {
        //     bag_ADRC.write("/uav1/prometheus/ADRCstate", ros::Time::now(), pos_controller_ne.adrc_state_pub);
        // }
        // // 在Land模式下，关闭bag文件，只运行一次
        // if (uav_command.Agent_CMD == prometheus_msgs::UAVCommand::Land && bag_close_flag == false)
        // {
        //     bag_ADRC.close();
        //     bag_close_flag = true;
        // }
    ...
    {
        // 设定期望值
        pos_controller_adrc.set_desired_state(desired_state);
        // 设定当前值
        pos_controller_adrc.set_current_state(uav_state);
        // 传入控制命令模式
        pos_controller_adrc.set_control_command(uav_command);
        // 传入遥控器控制模式
        pos_controller_adrc.set_control_state(uav_control_state);
        // // 传入负载位置信息
        // pos_controller_adrc.set_payload_position(payload_position);
        // 控制器更新
        Eigen::Vector4d u_att_from_pos_controller = pos_controller_adrc.update(50.0);
        // 遥控器拨杆从中间往下拨时，打开bag文件；反之则关闭bag文件
        if (last_control_state == CONTROL_STATE::RC_POS_CONTROL && control_state == CONTROL_STATE::COMMAND_CONTROL)
        {
            string file_path = "/bagfiles/ADRC_State.bag";
            string path = (getenv("HOME")+file_path).c_str();
            bag_ADRC.open(path, rosbag::bagmode::Write);
        ...
        {
            bag_ADRC.close();
            bag_write_flag = false;
        }
        // 条件满足时，写入ADRC变量至bag文件
        if(bag_write_flag == true && uav_command.Agent_CMD == prometheus_msgs::UAVCommand::Move)
        {
            bag_ADRC.write("/uav1/prometheus/ADRCstate", ros::Time::now(), pos_controller_adrc.adrc_state_pub);
            //bag_ADRC.write("/uav1/prometheus/Payloadstate", ros::Time::now(), pos_controller_adrc.payload_state_pub);
        }
        return u_att_from_pos_controller;
    }
}
...
void UAV_controller::set_hover_pose_with_rc()
{
    ...
    // 限制悬停位置在地理围栏之内(留有一定的余量，防止check_failsafe函数反复触发)
    if(Hover_position(0) < uav_geo_fence.x_min+0.05)
        Hover_position(0) = uav_geo_fence.x_min+0.05;
    if(Hover_position(0) > uav_geo_fence.x_max-0.05)
        Hover_position(0) = uav_geo_fence.x_max-0.05;
    if(Hover_position(1) < uav_geo_fence.y_min+0.05)
        Hover_position(1) = uav_geo_fence.y_min+0.05;
    if(Hover_position(1) > uav_geo_fence.y_max-0.05)
        Hover_position(1) = uav_geo_fence.y_max-0.05;
    if(Hover_position(2) > uav_geo_fence.z_max-0.05)
        Hover_position(2) = uav_geo_fence.z_max-0.05;
...
void UAV_controller::set_command_des_for_pos_controller()
{
    ...
    {
        //【Init_Pos_Hover】 移动到指定起飞位置
        // 此处z轴位置的起飞高度为相对解锁点的，改为绝对位置
        pos_des << Takeoff_position[0], Takeoff_position[1], Takeoff_height; // Takeoff_position + Eigen::Vector3d(0, 0, Takeoff_height);
        vel_des << 0.0, 0.0, 0.0;
        acc_des << 0.0, 0.0, 0.0;
        yaw_des = uav_command.yaw_ref;
    }
...
//void UAV_controller::uav_payload_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
//{
    // 负载位置订阅函数
    //payload_position = Eigen::Vector3d(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
    // payload_position(0) = msg->pose.position.x;
    // payload_position(1) = msg->pose.position.y;
    // payload_position(2) = msg->pose.position.z;
    // prometheus_msgs::UAVState payload_state;
    // payload_state.position = payload_position;
    // bag_UAVpos.write("/uav1/prometheus/ADRCstate", ros::Time::now(), payload_state);
//}
...
int UAV_controller::check_failsafe()
{
    ...
    {
        cout << RED << uav_name << ":----> Failsafe: Out of the geo fence！" << TAIL << endl;
        return 1;
    }
    /* 不判断是否有gps，舒沛玄更
    ...
    else if (payload_position == Eigen::Vector3d(0.0,0.0,0.0))
    {
        cout << RED << uav_name << ":----> Failsafe: Waiting for payload connection!" << TAIL << endl;
        return 3;
    }*/
    else
    {
...
void UAV_controller::printf_control_state()
{
    ...
    /*case POS_CONTOLLER::NE:
        ...
        break;*/
    case POS_CONTOLLER::ADRC:
        cout << GREEN << "Controller  : [ ADRC ] " << TAIL << endl;
        pos_controller_adrc.printf_result();
        break;
    }
    ...
    if (control_state == CONTROL_STATE::COMMAND_CONTROL)
    {
        ...
        case prometheus_msgs::UAVCommand::Init_Pos_Hover:
            ...
            cout << GREEN << "Init_Pos [X Y Z] : " << Takeoff_position[0] << " [ m ] " << Takeoff_position[1] << " [ m ] " << Takeoff_height << " [ m ] " << TAIL << endl;
            break;
    ...
    if (pos_controller == POS_CONTOLLER::PX4_ORIGIN)
    {
        ...
        else
        {
            ...
            cout << GREEN << "Thr_target [ 0-1 ] : " << px4_thrust_target << TAIL << endl;
             ...
             pos_controller == POS_CONTOLLER::UDE ||
             pos_controller == POS_CONTOLLER::ADRC)
    {
    ...
    // // 打印负载位置信息
    // cout << YELLOW << "payload_pos [X Y Z] : " << payload_position[0] << " [ m ] " << payload_position[1] << " [ m ] " << payload_position[2] << " [ m ] " << TAIL << endl;
}
